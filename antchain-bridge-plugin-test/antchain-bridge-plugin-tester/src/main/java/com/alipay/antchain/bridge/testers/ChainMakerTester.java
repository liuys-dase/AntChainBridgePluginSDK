package com.alipay.antchain.bridge.testers;

import cn.hutool.core.util.HexUtil;
import cn.hutool.core.util.StrUtil;
import com.alipay.antchain.bridge.abstarct.AbstractTester;
import com.alipay.antchain.bridge.exception.PluginTestToolException;
import com.alipay.antchain.bridge.plugins.spi.bbc.AbstractBBCService;
import org.chainmaker.pb.common.ChainmakerTransaction;
import org.chainmaker.pb.common.ContractOuterClass;
import org.chainmaker.pb.common.ResultOuterClass;
import org.chainmaker.sdk.utils.Utils;
import org.web3j.abi.TypeReference;
import org.web3j.abi.datatypes.Address;
import org.web3j.abi.datatypes.DynamicBytes;
import org.web3j.abi.datatypes.Type;
import org.web3j.abi.datatypes.Utf8String;
import org.web3j.abi.datatypes.generated.Bytes32;

import java.util.*;

import java.lang.reflect.Method;
import java.util.Arrays;
import java.util.Collections;

public class ChainMakerTester extends AbstractTester {

    private static final String CONTRACT_PREFIX_FORMAT = "000000000000000000000000{}";
    private static final long rpcCallTimeout = 10000;

    private static final String APP_NAME = "AppContract_{}";
    private static final String APP_METHOD_SET_PROTOCOL = "setProtocol";
    private static final String APP_METHOD_SEND_UNORDERED_MSG = "sendUnorderedMessage";
    private static final String APP_METHOD_GET_LAST_UNORDERED_MSG = "getLastUnorderedMsg";
    private static final String APP_BIN_BYTES = "";


    private static final String SDP_METHOD_GET_AM_CONTRACT = "getAmAddress";
    private static final String SDP_METHOD_GET_LOCAL_DOMAIN = "getLocalDomain";

    // TODO
    private static final String senderDomain = "chainmaker20240530T3.web3";
    private String senderSDPAddr = "b68a7b780fcc6c9bc986ccdc4f1ff1f56b9fe583";
    private static final String receiverDomain = "chainmaker20240530T4.web3";
    private String receiverSDPAddr = "194fb8d47c3e89a4d4178932642e174f70bb6c31";
    private static final String senderID = "dAC17F958D2ee523a2206206994597C13D831ec7";
    private static final String receiverID = "A0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48";
    private static final String msg = "awesome antchain-bridge";


    public ChainMakerTester(AbstractBBCService service) {

    }


    @Override
    public String getProtocol(String amContractAddr) throws PluginTestToolException {
        // TODO：`ChainMakerBBCServiceTest` 中没有包含该方法的实现
        return "";
    }

    @Override
    public byte[] deployApp(String protocolAddr) {
        return HexUtil.decodeHex(
                StrUtil.format("000000000000000000000000{}", receiverID)
        );
    }


    @Override
    public void waitForTxConfirmed(String txHash) throws InterruptedException {
        Thread.sleep(10000);
    }

    public void printResponseInfo(ResultOuterClass.TxResponse responseInfo) {
        StringBuilder hexString = new StringBuilder();
        for (byte b : responseInfo.getContractResult().getResult()) {
            hexString.append(String.format("%02x", b)); // 将每个字节转为两位的十六进制数
        }
        System.out.println("result (hex string): " + hexString.toString());
    }

    @Override
    public void sendMsgUnordered(AbstractBBCService service) throws Exception {

        senderSDPAddr = service.getContext().getSdpContract().getContractAddress();

        try {
            ResultOuterClass.TxResponse responseInfo;

            // 0-1. check sdp
            responseInfo = invokeContractMethod(
                    service,
                    senderSDPAddr,
                    SDP_METHOD_GET_AM_CONTRACT,
                    Arrays.<Type>asList(),
                    Collections.<TypeReference<?>>emptyList()
            );

            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()){
                throw new Exception("failed to get am contract address");
            }

            System.out.println("response info 0-1. check sdp: " + responseInfo);
            printResponseInfo(responseInfo);

            // 0-2. check sdp
            responseInfo = invokeContractMethod(
                    service,
                    senderSDPAddr,
                    SDP_METHOD_GET_LOCAL_DOMAIN,
                    Arrays.<Type>asList(),
                    Collections.<TypeReference<?>>emptyList()
            );
            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()) {
                throw new Exception("failed to get local domain");
            }

            System.out.println("response info 0-2. check sdp: " + responseInfo);

            // 1. sender deploy sender contract
            String senderAppContractName = Utils.calcContractName(StrUtil.format(APP_NAME, UUID.randomUUID().toString()));
            ContractOuterClass.Contract senderAppContract = invokeSetupContractMethod(service, APP_BIN_BYTES, senderAppContractName);
            String senderAppContractAddr = senderAppContract.getAddress();

            System.out.println("senderAppContractAddr: " + senderAppContractAddr);

            // 2. sender set protocol
            responseInfo = invokeContractMethod(
                    service,
                    senderAppContractAddr,
                    APP_METHOD_SET_PROTOCOL,
                    Arrays.<Type>asList(new Address(senderSDPAddr)),
                    Collections.<TypeReference<?>>emptyList()
            );

            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()) {
                throw new Exception("failed to set protocol");
            }

            // 3. receiver deploy receiver contract
            String receiverAppContractName = Utils.calcContractName(StrUtil.format(APP_NAME, UUID.randomUUID().toString()));
            ContractOuterClass.Contract receiverAppContract = invokeSetupContractMethod(service, APP_BIN_BYTES, receiverAppContractName);
            String receiverAppContractAddr = receiverAppContract.getAddress();

            System.out.println("receiverAppContractAddr: " + receiverAppContractAddr);

            // 4. receiver set protocol
            responseInfo = invokeContractMethod(
                    service,          // 传入 service 实例
                    receiverAppContractAddr,       // 合约地址
                    APP_METHOD_SET_PROTOCOL,       // 方法名称
                    Arrays.<Type>asList(new Address(receiverSDPAddr)),  // 输入参数列表
                    Collections.<TypeReference<?>>emptyList()  // 空的返回类型列表
            );

            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()) {
                throw new Exception("failed to set protocol");
            }

            Thread.sleep(10000);
            System.out.printf("wait for add acl, grantDomain: %s, grandIdentity: %s, ownerDomain: %s, ownerIdentity: %s\n",
                    senderDomain,
                    senderAppContractAddr,
                    receiverDomain,
                    receiverAppContractAddr);

            // 5. send msg
            responseInfo = invokeContractMethod(
                    service,          // 传入 service 实例
                    senderAppContractAddr,         // 合约地址
                    APP_METHOD_SEND_UNORDERED_MSG, // 方法名称
                    Arrays.<Type>asList(
                            new Utf8String(receiverDomain),  // 第一个参数
                            new Bytes32(str2bytes32(StrUtil.format(CONTRACT_PREFIX_FORMAT, receiverAppContractAddr))),  // 第二个参数
                            new DynamicBytes(msg.getBytes())  // 第三个参数
                    ),
                    Collections.<TypeReference<?>>emptyList()  // 空的返回类型列表
            );


            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()) {
                throw new Exception("failed to send unordered msg");
            }

            System.out.println("responseInfo1\n" + responseInfo.toString());

            printResponseInfo(responseInfo);

            Thread.sleep(10000);

            // 6. get msg
            responseInfo = invokeContractMethod(
                    service,          // 传入 service 实例
                    receiverAppContractAddr,       // 合约地址
                    APP_METHOD_GET_LAST_UNORDERED_MSG,  // 方法名称
                    Arrays.<Type>asList(),         // 空的输入参数列表
                    Collections.<TypeReference<?>>emptyList()  // 空的返回类型列表
            );

            System.out.println("responseInfo2\n" + responseInfo.toString());

            if (ResultOuterClass.TxStatusCode.SUCCESS != responseInfo.getCode()) {
                throw new Exception("failed to get last unordered msg");
            }

            if (!new String(responseInfo.getContractResult().getResult().toByteArray()).contains(msg)) {
                throw new Exception("response info does not contain msg");
            }

//            System.out.println("responseInfo\n" + new String(responseInfo.getContractResult().getResult().toByteArray()));

        } catch (Exception e) {
            throw new RuntimeException(
                    StrUtil.format("failed to send unordered msg"), e
            );
        }
    }

    private byte[] str2bytes32(String str) {
        if (str.length() != 64) {
            throw new RuntimeException("str {} length is not 64");
        }
        byte[] byteArray = new byte[32];
        for (int i = 0; i < byteArray.length; i++) {
            int index = i * 2;
            // 将每两个十六进制字符转换为一个 byte
            int value = Integer.parseInt(str.substring(index, index + 2), 16);
            byteArray[i] = (byte) value;
        }
        return byteArray;
    }

    private static ResultOuterClass.TxResponse invokeContractMethod(
            AbstractBBCService service,
            String contractAddr,
            String methodName,
            List<Type> inputParams,
            List<TypeReference<?>> outputParams
    ) throws Exception {
        try {
            // 获取 invokeContract 方法
            Method invokeContractMethod = service.getClass().getMethod(
                    "invokeContract",
                    String.class,
                    String.class,
                    List.class,
                    List.class
            );

            // 调用方法并接收返回值
            Object result = invokeContractMethod.invoke(
                    service,
                    contractAddr,        // 合约地址
                    methodName,          // 方法名称
                    inputParams,         // 输入参数列表
                    outputParams         // 输出参数类型列表
            );

            // 检查返回值是否是 ResultOuterClass.TxResponse 类型
            if (result instanceof ResultOuterClass.TxResponse) {
                return (ResultOuterClass.TxResponse) result;
            } else {
                throw new Exception("Returned object is not of type ResultOuterClass.TxResponse");
            }
        } catch (Exception e) {
            throw new Exception("Error invoking invokeContract method", e);
        }
    }


    private ContractOuterClass.Contract invokeSetupContractMethod(AbstractBBCService service, String appBinBytes, String receiverAppContractName) throws Exception {
        try {
            // 获取 setupContract 方法，参数类型分别是 String 和 String
            Method setupContractMethod = service.getClass().getMethod("setupContract", String.class, String.class);

            // 调用 setupContract 方法并接收返回值
            Object result = setupContractMethod.invoke(service, appBinBytes, receiverAppContractName);

            // 检查返回值是否是 ContractOuterClass.Contract 类型
            if (result instanceof ContractOuterClass.Contract) {
                return (ContractOuterClass.Contract) result;
            } else {
                throw new Exception("Returned object is not of type ContractOuterClass.Contract");
            }
        } catch (Exception e) {
            throw new Exception("Error invoking setupContract method", e);
        }
    }

    @Override
    public void sendMsgOrdered(AbstractBBCService service) throws Exception {

    }

    @Override
    public String getAmAddress(AbstractBBCService service) throws Exception {
        // TODO：`ChainMakerBBCServiceTest` 中没有包含该方法的实现
        return "";
    }

    @Override
    public byte[] getLocalDomain(AbstractBBCService service) throws Exception {
        return new byte[0];
    }

    @Override
    public void checkTransactionReceipt(AbstractBBCService service, String txHash) throws Exception {
        getTransactionInfoByTxId(service, txHash, rpcCallTimeout);
    }

    private ChainmakerTransaction.TransactionInfo getTransactionInfoByTxId(AbstractBBCService chainMakerBBCService, String txHash, long rpcCallTimeout) throws Exception {
        try {
            // 通过反射获取 chainMakerBBCService 的 getChainClient 方法
            Method getChainClientMethod = chainMakerBBCService.getClass().getMethod("getChainClient");

            // 调用 getChainClient 方法，获取 ChainClient 对象
            Object chainClient = getChainClientMethod.invoke(chainMakerBBCService);

            // 确保 chainClient 对象不为空
            if (chainClient == null) {
                throw new Exception("ChainClient is null");
            }

            // 获取 chainClient 的 getTxByTxId 方法
            Method getTxByTxIdMethod = chainClient.getClass().getMethod("getTxByTxId", String.class, long.class);

            // 调用 getTxByTxId 方法，获取 TransactionInfo 对象
            Object transactionInfo = getTxByTxIdMethod.invoke(chainClient, txHash, rpcCallTimeout);

            // 确保 transactionInfo 是 ChainmakerTransaction.TransactionInfo 类型
            if (transactionInfo instanceof ChainmakerTransaction.TransactionInfo) {
                return (ChainmakerTransaction.TransactionInfo) transactionInfo;
            } else {
                throw new Exception("Returned object is not of type ChainmakerTransaction.TransactionInfo");
            }
        } catch (Exception e) {
            throw new Exception("Error invoking getTxByTxId method", e);
        }
    }
}
